...
private:
    Villager **villagers;
    unsigned maxElements, currentElements;
public:
    Household(unsigned maxElements, unsigned initSize)
    {
        this->maxElements=maxElements;
        this->currentElements=initSize;
        villagers=new Villager * [this->currentElements];
        for(unsigned i=0; i<this->currentElements; i++)
            villagers[i]=new Villager();
    }
    void nextYear()
    {
        for(unsigned i=0; i<currentElements; i++)
            villagers[i]=zwiekszwiek();
    }
};
